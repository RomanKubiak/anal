...
// @return void
//
void FifteenStep::setMidiHandler(MIDIcallback cb, void *midi_handler_user)
{
  // store the passed callback
  _midi_cb = cb;
  _midi_handler_user = midi_handler_user;
}
...
// @return void
//
void FifteenStep::setStepHandler(StepCallback cb, void *step_handler_user)
{
  // store the passed callback
  _step_cb = cb;
  _step_handler_user = step_handler_user;
}

FifteenStepNote *FifteenStep::getStep(int step)
{
  for(int i=0; i < _sequence_size; ++i)
    if (_sequence[i].step == step && _sequence[i].channel > 0)
      return(&_sequence[i]);
  return (NULL);
}

void FifteenStep::setStep(int step, byte channel, byte pitch, byte velocity)
{
  int position = step;
  bool added = false;
...
void FifteenStep::panic()
{
  ...
  for(int i=0; i < 16; ++i)
  {
    ...
    if(_midi_cb)
      _midi_cb(i, 0x7B, 0x0, 0x0, _midi_handler_user);
  }
...
void FifteenStep::_step()
{
  ...
  if(_step_cb)
    _step_cb(_position, last, _step_handler_user);
...
void FifteenStep::_tick()
{
  ...
  // tick
  _midi_cb(0x0, 0xF8, 0x0, 0x0, _midi_handler_user);
...
void FifteenStep::_loopPosition()
{
  ...
  // send position
  _midi_cb(0x0, 0xF2, 0x0, _position, _midi_handler_user);
