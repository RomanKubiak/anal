...
// a MIDI library.
//
typedef void (*MIDIcallback) (byte channel, byte command, byte arg1, byte arg2, void *user);
...
// that will be used.
//
typedef void (*StepCallback) (int current, int last, void *user);
...
class FifteenStep
{
  public:
    ...
    byte  getSteps() { return (_steps); }
    void  increaseTempo();
    void  decreaseTempo();
    void  increaseShuffle();
    void  decreaseShuffle();
    void  setMidiHandler(MIDIcallback cb, void *midi_handler_user = NULL);
    void  setStepHandler(StepCallback cb, void *step_handler_user = NULL);
    void  setNote(byte channel, byte pitch, byte velocity);
    void  setStep(int step, byte channel, byte pitch, byte velocity);
    FifteenStepNote *getStep(int step);
  private:
    MIDIcallback      _midi_cb;
    StepCallback      _step_cb;
    FifteenStepNote*  _sequence;
    void *            _step_handler_user;
    void *            _midi_handler_user;
    bool              _running;
    int               _sequence_size;
    int               _tempo;
    byte              _steps;
